...
func SpawnAuditor(collector *Collector) string {
	...
	cd CTAuditor
	./main %s %s %s %s %s > log.txt`, strconv.Itoa(int(collector.RunTasks[collector.CurrentTask].TotalClients)), strconv.Itoa(int(collector.RunTasks[collector.CurrentTask].MaxSitOut)), "80", collector.CollectorIP, int(collector.RunTasks[collector.CurrentTask].Shuffler))
